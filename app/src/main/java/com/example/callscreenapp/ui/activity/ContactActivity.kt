...
class ContactActivity : AppCompatActivity(), ContactItemClickListener, NetworkChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSubmit.setOnClickListener() {
            ...
            downloadImageAndSave(this, urlItem, "contact_background_theme")
            downloadImageAndSave(this, urlAvatar, "contact_avatar_theme")
            downloadImageAndSave(this, urlBtnCallGreen, "contact_icon_answer")
            downloadImageAndSave(this, urlBtnCallRed, "contact_icon_reject")

            realm.writeBlocking {
                val query = query<ContactDb>().find()
                delete(query)
                saveContact.forEach { contact ->
                    copyToRealm(ContactDb().apply {
                        contactPhone = contact.phoneNumber
                        contactName = contact.name
                        avatarTheme = "contact_avatar_theme"
                        backgroundTheme = "contact_background_theme"
                        btnAnswer = "contact_icon_answer"
                        btnReject = "contact_icon_reject"
                    })
                }
            }
            ...
            itemsNew.forEach { item ->
                Log.d("itemsNew", item.contactPhone)
            }
